...
KIND_VERSION ?= focal-arm
KIC_VERSION ?= $(shell egrep "Version =" pkg/drivers/kic/types.go | cut -d \" -f2)-arm
...
kind-base-image: ## builds the base image used for kind.
	...
	docker buildx build --platform=linux/armv7 --load -f ./deploy/kindbase/Dockerfile -t local/kindbase:$(KIND_VERSION)-snapshot ./deploy/kindbase
...
kic-base-image: kind-base-image ## builds the base image used for kic.
	...
	docker buildx build --platform=linux/armv7 --load -f ./deploy/kicbase/Dockerfile -t local/kicbase:$(KIC_VERSION)-snapshot  --build-arg COMMIT_SHA=${VERSION}-$(COMMIT) --cache-from $(KIC_BASE_IMAGE_GCR) --target base ./deploy/kicbase
